...
function consulPlaca() {
  ...
  if (numplaca == "WWW404") {
    ...
    if (tipoDocumentoID == "2") {
      ...
      if (rolAsesor == 19) {
        restriccion =
          "Lo sentimos, no puedes realizar cotizaciones para personas jurídicas por este cotizador. Para hacerlo cotiza como persona natural con los datos del representante legal o comunicate con el Equipo de Asesores Freelance de Grupo Asistencia, quienes podrán ayudarte a cotizar de manera manual con diferentes aseguradoras.";
      } else {
        restriccion =
          "Lo sentimos, no puedes realizar cotizaciones para personas jurídicas (con NIT) por este cotizador. Cotiza como persona natural con los datos del representante legal.";
      }
      Swal.fire({
        icon: "error",
        title: "Lo sentimos",
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (intermediario != 78) {
              ...
              cont.push(
                fetch(
                  "https://grupoasistencia.com/motor_webservice_tst2/AXA_tst?callback=myCallback",
                  requestOptions
                )
                  .then((res) => {
                    if (res.status === 500) {
              ...
              const aseguradoras_sin_convenio = [
                "Allianz",
                "Liberty",
                "Previsora",
                "SBS",
              ...
              aseguradoras_sin_convenio.forEach((entidad) => {
                mostrarAlertarCotizacionFallida(
                  entidad,
                  "Entidad sin convenio con el intermediario"
                );
              });
