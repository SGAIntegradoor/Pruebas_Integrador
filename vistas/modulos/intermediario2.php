...
<?php
require_once 'controladores/intermediario.controller.php';
      ...
      <div class="box-body">
        <table class="table table-bordered table-striped  dt-responsive tablas-cotizaciones" width="100%">
          ...
          <tbody id="contenidoInter">
            ...
            $intermediarios = IntermediarioController::ctrGetIntermediarios();

            $intermediarios_json = json_encode($intermediarios);

            // Imprimir los datos dentro de una etiqueta <script> en tu HTML
            echo '<script>var intermediarios = ' . $intermediarios_json . ';
            console.log(intermediarios);
            </script>';
            
            echo '<script>'.json_encode($intermediarios).'</script>';
             foreach ($intermediarios as $key => $value) {
             }


            
            ?>
             <!-- for (i = 0; i < data.length; i++) { contenido +="<tr><td>" + data[i]['id_Intermediario'] + "</td><td>" + data[i]["num_documento"] + "</td><td>" + data[i]["nombre"] + "</td><td>" + data[i]["nombre_representante"] + "</td> <td>" + data[i]["celular"] + "</td> <td>" + data[i]["correo"] + "</td><td>" + data[i]["ciudad"] + "</td><td>" + data[i]["contacto"] + "</td>" if (data[i]["intermediario_Estado"]==0) { contenido +="<td><button class='btn btn-success btn-xs btnActivar' onclick='cambiarEst(" + data[i]['id_Intermediario'] + ", " + data[i]['intermediario_Estado'] + ", 1)'>Activo</button></td>" ; } else { contenido +="<td><button class='btn btn-danger btn-xs btnActivar'onclick='cambiarEst(" + data[i]['id_Intermediario'] + ", " + data[i]['intermediario_Estado'] + ", 0)'>Bloqueado</button></td>" ; } contenido +="<td><div><button class='btn btn-primary btnEditarUsuario' onclick='Editiinter(" +data[i]['id_Intermediario']+")' idUinter='" + data[i][' id_Intermediario'] + "' data-toggle='modal' data-target='#modalEditarIntermediario'><i class='fa fa-pencil'></i></button> <button class='btn btn-danger btnEliminarUsuario' onclick='elimiNarInter(" +data[i]['id_Intermediario']+")' idUsuario='" + data[i][' id_Intermediario'] + "'><i class='fa fa-times'></i></button></div></td></tr>" }   -->
